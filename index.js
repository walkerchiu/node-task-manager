...
class TaskManager extends EventEmitter {
    ...
    async run() {
        ...
        while (this.isRunning) {
            if (this.taskQueue.length > 0) {
                ...
                if (taskExecution) {
                    console.log(`Dequeue Task: ${taskExecution.taskId}, Service: ${taskExecution.service}`);
                    let success = await this.processTaskExecution(taskExecution);
                    if (success) {
                        await this.updateExecutionStatus(taskExecution, ExecutionStatusEnum.SUCCESS);
                    } else {
                        await this.updateExecutionStatus(taskExecution, ExecutionStatusEnum.FAILED);
                    }
                }
            }
    ...
    async startMonitoring() {
        this.interval = setInterval(() => {
            if (this.taskQueue.length === 0 && Object.keys(this.taskTimers).length === 0) {
